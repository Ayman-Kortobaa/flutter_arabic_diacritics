...
///
/// A [ArabicTextSpan] object can be styled using its [style] property. The style will
/// be applied to the [text] and the [children].
///
/// A [ArabicTextSpan] object can just have plain text, or it can have children
/// [ArabicTextSpan] objects with their own styles that (possibly only partially)
/// override the [style] of this object. If a [ArabicTextSpan] has both [text] and
/// [children], then the [text] is treated as if it was an un-styled [ArabicTextSpan]
/// at the start of the [children] list. Leaving the [ArabicTextSpan.text] field null
/// results in the [ArabicTextSpan] acting as an empty node in the [InlineSpan] tree
/// with a list of children.
///
/// To paint a [ArabicTextSpan] on a [Canvas], use a [TextPainter]. To display a text
...
///
/// The [ArabicTextSpan.text] will be used as the semantics label unless overridden
/// by the [ArabicTextSpan.semanticsLabel] property. Any [PlaceholderSpan]s in the
/// [ArabicTextSpan.children] list will separate the text before and after it into two
/// semantics nodes.
///
/// See also:
///
...
///  * [TextPainter], a class for painting [ArabicTextSpan] objects on a [Canvas].
@immutable
class ArabicTextSpan extends InlineSpan
    implements HitTestTarget, MouseTrackerAnnotation {
  /// Creates a [ArabicTextSpan] with the given values.
  ///
  ...
  /// children.
  ///
  /// Modifying the list after the [ArabicTextSpan] has been created is not supported
  ...
  ///
  /// [ArabicTextSpan] itself does not implement hit testing or cursor changing.
  /// The object that manages the [ArabicTextSpan] painting is responsible
  /// to return the [ArabicTextSpan] in its hit test, as well as providing the
  /// correct mouse cursor when the [ArabicTextSpan]'s mouse cursor is
  /// [MouseCursor.defer].
  ...
  /// An alternative semantics label for this [ArabicTextSpan].
  ///
  ...
  ///
  /// ```dart
  /// ArabicTextSpan(text: r'$$', semanticsLabel: 'Double dollars')
  /// ```
  ...
  /// [TextPainter] class to paint [ArabicTextSpan] objects onto [Canvas]
  /// objects.
  @override
  void build(
  ...
  /// Walks this [ArabicTextSpan] and its descendants in pre-order and calls [visitor]
  ...
  ///
  /// ```dart
  /// assert(myArabicTextSpan.debugAssertIsValid());
  /// ```
  @override
        ...
        for (final InlineSpan child in children!) {
          if (child == null) {
            throw FlutterError.fromParts(<DiagnosticsNode>[
              ErrorSummary('ArabicTextSpan contains a null child.'),
              ErrorDescription(
                'A ArabicTextSpan object with a non-null child list should not have any nulls in its child list.',
              ),
              toDiagnosticsNode(
  ...
  RenderComparison compareTo(InlineSpan other) {
    ...
    final ArabicTextSpan textSpan = other as ArabicTextSpan;
  ...
  bool operator ==(Object other) {
    ...
    return other is ArabicTextSpan &&
        other.text == text &&
  ...
  @override
  String toStringShort() => objectRuntimeType(this, 'ArabicTextSpan');
